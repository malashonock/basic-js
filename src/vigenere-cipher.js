...
export default class VigenereCipheringMachine {
  ...
  encrypt(message, key) {
    ...
    const encryptedMessage = message.toUpperCase().split('').map((letter, i) => {
      if (!letter.match(/[A-Z]/)) {
        return letter;
      }
  ...
  decrypt(encryptedMessage, key) {
    ...
    const message = encryptedMessage.toUpperCase().split('').map((encryptedLetter, i) => {
      ...
      return String.fromCharCode(CHARCODE_A + ((e - k + ALPHA_COUNT) % ALPHA_COUNT));
    }).join('');
